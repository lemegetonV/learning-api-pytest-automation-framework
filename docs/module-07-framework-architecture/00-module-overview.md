...
| --- | --- | --- |
| Config | `src/config/settings.py`, `.env.example` | Centralized URLs, timeout, environment name, log level |
| API client | `src/api_client/client.py` | URL joining, session reuse, default headers, timeout defaults |
| Test fixtures | `tests/conftest.py` | Shared `api_client` fixture wired from settings |
| Learning tests | `tests/learning/test_07_framework/` | Proves config and client behavior |
...
1. `01-why-framework-architecture.md`
2. `02-config-management.md`
3. `03-api-client.md`
4. `04-sessions-and-fixtures.md`
5. `exercises.md`
...
- `python-dotenv` is active in `requirements.txt`.
- Settings exist in `src/config/settings.py`.
- `APIClient` exists in `src/api_client/client.py`.
- `api_client` fixture exists in `tests/conftest.py`.
